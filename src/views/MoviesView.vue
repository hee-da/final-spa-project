        ...
        <ul>
            <li v-for="movie in movies" :key="movie.id" class="movie-item">
                ...
                <button @click="favoriteStore.toggleFavorite(movie)" class="fav-btn">
                    {{ favoriteStore.favoriteMovies.some((m) => m.id === movie.id) ? '💔 찜 취소' : '🤍 찜하기' }}
                </button>
            </li>
        </ul>
    </main>
...
.fav-btn { padding: 8px 12px; border: none; border-radius: 4px; background-color: #ffdde1; cursor: pointer; font-weight: bold; transition: 0.2s; }
